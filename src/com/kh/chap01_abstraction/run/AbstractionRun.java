...
public class AbstractionRun {
	 ...
	 * 			@자바의 입장에서는 메모리 영역 중 heap 영역에 만들어지는 모든 것들
	 *  		 현실세계에서는 객체들간의 상호작용으로 돌아감
	 *  
	 * 3. 객체지향 프로그래밍: 현실세계의 객체들간의 상호작용을
	 * 					   프로그래밍을 통해 가상세계로 구현하는 과정
	 * 
	 * 4. 구현하고자 하는 프로그램 상의 필요한 객체들을 만들기 위해서(즉, 생성하기 위해서)
	 * 	   : 클래스라는 틀을 먼저 만들어야함! (클래스란? 각 객체들의 속성(정보)들을 담아낼 그릇같은 존재)
	 * 		>> 어떻게? ==> 추상화 + 캡슐화 접목해서
	 * 
	 * 5. 추상화 과정
	 * 		1) 내가 구현하고자 하는 프로그램에서필요한 객체들을 생각해볼 것
	 ...
	 * 			2) 공통적인 속성 및 기능들 (이름, 나이, 주소, 전화번호, 키, 몸무게, 달리기 속도, 학년, 반, 국어점수, 수학점수, 영어점수, 머리색깔, 발사이즈, 먹는다, 똥싼다, 잠잔다,...)
	 * 			3) 학생 '인적사항'을 관리하는 프로그램은?: 이름, 나이, 주소, 전화번호,학년, 반
	 * 			      학생 '성적'을 관리하는 프로그램은?: 이름, 학년, 반, 국어점수, 수학점수, 영어점수
	 * 			      학생 '건강'을 관리하는 프로그램은?: 이름, 학년, 반, 키, 몸무게, 달리기 속도, 먹는다, 똥싼다,...
	 * 				
	 * 				결국 추려진 것이 '이름, 나이, 키'라는 가정 하에
	 * 			4) 이름: String name;
	 * 			     나이: int age;
	 * 			     키: double height;
	 * 
	 * 6. 위에서 선정된 항목들로 본격적인 프로그래밍 시작 ...
	 * 
	 * 
	 * 	1) "변수"만으로 프로그래밍을 한다면?
	 * 		
	 ...
	 * 		... 등등의 수백 명에 해당하는 학생들을 만들어야 됨.
	 * 
	 * 		어떤 문제가 발생할까?
	 * 		수백명에 해당하는 학생들을 만들 때 => 수천개의 변수가 만들어짐! 하나하나 모두 기억할 수 없음.
	 * 
	 * 	2) "배열"로 프로그래밍을 한다면?
	 * 		
	 ...
	 * 
	 * 		어떤 문제가 발생할까?
	 * 		기존의 학생 정보를 지워야 할 때 뒤의 정보들알 앞으로 땡기는 과정을 진행해야 하는데, 혹시라도 실수하면 학생의 정보가 뒤바뀔 수 있음
	 * 		=> 새로운 학생 정보를 기록해야 할 때 배열의 크기를 변경할 수 없기때문에 새로운 크기의 배열을 만들어서 기존 것을 복사해야함. 과정이 복잡해짐.
	 * 
	 * 	3) 그래서 나온 개념이 => 구조체
	 * 		
	 ...
	 * 		Student 클래스를 만들 것. => 객체의 정보를 보관할 그릇과도 같은 이와 같은 클래스들은 => model.vo 패키지에 보관
	 * 
	 * 
	 * 
	 * 
	...
	public static void main(String[] args) {
		...
		// 자바에서의 객체로 만들기 위해서 new라는 키워드를 통해 Heap 영역에 생성을 먼저 해야함! (공간 할당)
		Student hong = new Student(); // 객체 생성 구문
		// 여기까지는 Heap 영역에 할당된 공간에는 JVM의 초기값들이 세팅되어있을 것.
		...
		System.out.println(hong); // 의도한 바와 다르게 출력됨
		 ...
		 * => 외부에서 함부로 값을 변질시키거나, 조회권한이 없는 사람이 값을 쉽게 가져올 수 있는 문제가 발생함!
		 * 	  >> 캡슐화 작업을 할 것임!!
		 */
